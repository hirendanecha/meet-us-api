...
exports.channelNotificationEmail = async (userData) => {
  ...
  let msg = `You have been assign in MeetUsTube channel by the MeetUs Tube Admin.
             To access your channel, log into your MeetUs Tube account,click on the
             MeetUs Tube icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `MeetUs Tube has approved your Community account.`;
    } else {
      msg = `MeetUs Tube has approved your Community account.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
...
exports.channelCreationMail = async (adminMail, userName) => {
  let redirectUrl = "https://admin.meetus.tube/community";
  let msg = `${userName} has create community on meetus.tube.`;
  const mailObj = {
    email: adminMail,
